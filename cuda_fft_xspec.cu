	...
	char	**argv )		// Pointer to Arguments
{
	...
	FILE	*file_ptr[4];				// File Pointer to write
	char	fname[16];					// File Name [YYYYDOYHHMMSSIF]
	...
	while(param_ptr->validity & ACTIVE){
		...
		if(rec_index == 0){
			for(index=0; index<Nif; index++){
				sprintf(fname, "%04d%03d%02d%02d%02d%02d",
					param_ptr->year, param_ptr->doy,
					param_ptr->hour, param_ptr->min,
					param_ptr->sec, index);
...
*/
	...
	cudaFree(cuk5data_ptr); cudaFree(cuRealData); cudaFree(cuSpecData); cudaFree(cuPowerSpec);
